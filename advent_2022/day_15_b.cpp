	...
	REP(y, 5*y_size){		
		REP(x, 5*x_size){
			if(x>=x_size|| y>=y_size){
				int diff = x / x_size + y / y_size;
				int parent_idx_x = x % x_size;
				int parent_idx_y = y % y_size;

				int new_elem = vec[parent_idx_y][parent_idx_x] + diff;
				new_elem = new_elem / 10 + new_elem % 10;

				// if y
				vec[y].push_back(new_elem);
			}			
		}
	}
